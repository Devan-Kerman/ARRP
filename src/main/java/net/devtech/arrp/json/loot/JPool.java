...
public class JPool implements Cloneable {
	...
	public JPool condition(JCondition condition) {
		if (this.conditions == null) this.conditions = new ArrayList<>(1);
		this.conditions.add(condition);
		return this;
	}

	public JPool function(JFunction function) {
		if (this.functions == null) this.functions = new ArrayList<>(1);
	...
	public static class Serializer implements JsonSerializer<JPool> {
		...
		public JsonElement serialize(JPool src, Type typeOfSrc, JsonSerializationContext context) {
			...
			if (src.conditions != null) obj.add("conditions", context.serialize(src.conditions));
			if (src.functions != null) obj.add("functions", context.serialize(src.functions));
			if (src.entries != null) obj.add("entries", context.serialize(src.entries));
			if (src.rolls != null) obj.addProperty("rolls", src.rolls);
			if (src.roll != null) obj.add("rolls", context.serialize(src.roll));
